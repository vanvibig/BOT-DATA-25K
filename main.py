...
@click.option('-bs', '--batch-size', default=128, help='Batch size for training on minibatches',)
...
def train(data_corpus, batch_size, num_epochs, learning_rate, inference_mode):
    ...
    src_vocab_size = len(metadata['idx2w']) # 10002 (0~10001)
    emb_dim = 1024
    ...
    start_id = src_vocab_size  # 10002
    end_id = src_vocab_size + 1  # 10003
    ...
    def vn_inf(seed):
        a = []
        seed.lower()
        a.append(ViTokenizer.tokenize(seed))
        a = [ c.split(' ') for c in a ]
        for i in range(len(a)):
            a[i] = [j.replace('_', ' ') for j in a[i]]
        return a[0]
    ...
    """
    def inference(seed):
        seed_id = [word2idx.get(w, unk_id) for w in vn_inf(seed)]
        print("---------------------")
        print(seed_id)
        print("---------------------")
    ...
    if inference_mode:
        """
        # Ghi file cac cau tl -> BLEU
        fx = open("kq-botchat.txt", "w", encoding='utf-8', errors='ignore')
        seeds = open('question.txt', encoding='utf-8', errors='ignore').read().split('\n')
        ...
        seeds = ["chúc bạn ngày mới tốt lành",
        ...
        for epoch in range(num_epochs):
            ...
            # Hiển thị lỗi tb sau mỗi epoch
            print('Epoch [{}/{}]: loss {:.4f}'.format(epoch + 1, num_epochs, total_loss / n_iter))
            
            # inference sau mỗi epoch
            # Mỗi câu hỏi sẽ in ra 5 câu tl
            for seed in seeds:
                print("Query >", seed)
                for _ in range(5):
                    sentence = inference(seed)
